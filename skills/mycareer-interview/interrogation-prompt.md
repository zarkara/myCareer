# Reusable Career Interrogation Prompt

**Purpose:** paste this at the start of a fresh session to reconstruct one role, one system, or one claim set until every fact is confirmed, attributed, and mapped to the job families it actually supports. The assistant works as a memory scaffold: I describe the system and the space we were operating in, it lays out what *typically* applied there, and I confirm, correct, or reject each item. Works for engineering, product, architecture, and delivery-management tracks.

**What it produces:** entries in a **myCareer** corpus — the high-fidelity career record that résumés, profiles, letters, and interview answers are all compressed from. Capture happens at autobiography depth, above every output format, because compression only runs one direction. See [docs/mycareer.md](docs/mycareer.md) and [schema/mycareer.schema.json](schema/mycareer.schema.json).

**How to use:** fill the variables, paste the whole block, then start describing a system. Stop whenever you want; the assistant hands back whatever is confirmed so far, as corpus entries that merge into what already exists.
